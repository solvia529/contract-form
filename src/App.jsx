        ...
        <div style={S.bd}>
          ...
          {stp===2&&<>
            ...
            {py!=='デンタルローン'&&<>
              ...
              {tr&&<p style={{fontSize:12,color:nextDay?'#8a5a00':'#cc0000',textAlign:'center',marginTop:8,fontWeight:700,lineHeight:1.6}}>振込・振込明細のLINE送付は{nextDay?'来院前':(dl||'3日前')}<br/>までにお願いします</p>}
        ...
        <div style={S.nav}>
          ...
          {stp<STPS.length-1
            ...
            <button style={{...S.nb,flex:1,opacity:submitting?0.6:1}} disabled={submitting} onClick={async()=>{
                setSubmitting(true);
                try{
                  await fetch(import.meta.env.VITE_GAS_URL,{
                    method:'POST',
                    mode:'no-cors',
